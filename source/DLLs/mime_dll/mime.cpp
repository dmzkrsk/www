#include "StdAfx.h"
#define MIMELinkage __declspec (dllexport)
#include "mime.h"
...
BOOL CMIMETypes::IsExtension(char* type, char* ext)
{
	...
	IsError=FALSE;
	mimetype::iterator it;
	if(MIMETypeArray.find(string(type))==MIMETypeArray.end()) return TRUE;
	it=MIMETypeArray.lower_bound(string(type));
	while(it!=MIMETypeArray.upper_bound(string(type)))
	{
		char* tdata=(char* )(*it).second.data();
		if (!stricmp(ext, tdata)) return TRUE;
		it++;
	}
	return FALSE;
}
